...
static void addfd( int epollfd, int fd )
{
    epoll_event event;
    event.data.fd = fd;
    event.events = EPOLLIN | EPOLLET;   //对应fd可读(包括正常关闭) | 边缘触发
    // 注册要监听的事件类型(监听fd的event事件)
    ...
    : m_listenfd( listenfd ), m_process_number( process_number ), m_idx( -1 ), m_stop( false )
{
    ...
    // create的时候process_number默认是8
    for( int i = 0; i < process_number; ++i )
    {
        // socketpair()函数用于创建一对无名的、相互连接的套接子。 
        // 如果函数成功，则返回0，创建好的套接字分别是m_pipefd[0]和m_pipefd[1]；否则返回-1，错误码保存于errno中。 
        // 这对套接字可以用于全双工通信，每一个套接字既可以读也可以写。
        // 例如，可以往m_pipefd[0]中写，从m_pipefd[1]中读；或者从m_pipefd[1]中写，从m_pipefd[0]中读； 
        ...
        else
        {
            ...
            // 子进程在池中的序号
            // 因为fork时会复制父进程的所有数据段堆栈段，所以每个子进程都有独有的m_idx等数据
            m_idx = i;
            // 子进程不用fork
            break;
        }
...
void processpool< T >::run_child()
{
    ...
    // 独属于自己的m_idx,子进程使用的是m_pipefd[1]与父进程通信
            ...
            else if( ( sockfd == sig_pipefd[0] ) && ( events[i].events & EPOLLIN ) )
            {
                ...
                else
                {
                    for( int i = 0; i < ret; ++i )
                    {
                        switch( signals[i] )
                        {
                            // 父子进程都能从sig_pipefd[0]收到SIGINT
                            // 子进程能做的就是退出循环
                            // 父进程负责kill(pid,SIGTERM),SIGTERM信号要求进程正常退出，该信号可以阻塞
                            case SIGINT:
                            {
                                m_stop = true;
                                break;
